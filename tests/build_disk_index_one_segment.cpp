...
int main(int argc, char** argv) {
  ...
  try {
    ...
    desc.add_options()("without_graph", po::value<bool>(&only_pq)->default_value(false),
                       "Only do PQ training, no graph construction");
    if(only_pq) {
      diskann::cout<< "Only do PQ training, no graph construction.";
    } else {
      diskann::cout<< "normal training.";
    }
  ...
  splite_fname_stream<<data_path<<"_part"<<0;
  std::string split_fname =  splite_fname_stream.str();

  size_t num = std::min(data_size_per_segment, base_num);
  ...
  splite_fname_stream.str("");
  splite_fname_stream<<index_path_prefix<<"_part"<<0;                  
  std::string split_index_path_prefix = splite_fname_stream.str();
  diskann::cout<<" index building to index file"<<split_index_path_prefix<<std::endl;
  auto s = std::chrono::high_resolution_clock::now();
  try {
    if (data_type == std::string("int8")) {
      diskann::save_bin<int8_t>(split_fname.c_str(), (int8_t*)buffer.data(), num, base_dim);
      diskann::build_disk_index<int8_t>(
          split_fname.c_str(), split_index_path_prefix.c_str(), params.c_str(), metric, only_pq);
    }
